...
import { FaArrowLeft, FaQrcode, FaMoneyBillWave } from "react-icons/fa";
...
const Cart = ({ cart, clearCart, updateQuantity, restaurantId, tableNum, setTableNum }) => {
    ...
    // --- SAFETY CHECK: Get ID from props OR storage ---
    ...
    const handlePlaceOrder = async () => {
        ...
        // 2. CRITICAL: Stop if no Restaurant ID
        if (!finalRestaurantId) {
            alert("❌ Session Expired. Please verify by scanning the QR code again.");
            return; 
        }
        ...
        const orderData = {
            ...
            owner: finalRestaurantId, // This MUST be a valid ID
            items: cart.map(item => ({
                dishId: item._id, // This MUST be a valid ID
                name: item.name,
        ...
        console.log("SENDING ORDER:", orderData); // 👈 Check your console for this!
        ...
        try {
            ...
            const msg = error.response?.data?.message || "Server Rejected Request (400)";
        ...
        <div style={{ minHeight: '100vh', background: '#080808', color: 'white', padding: '20px', paddingBottom: '120px', maxWidth: '480px', margin: '0 auto', fontFamily: 'sans-serif' }}>
            ...
            <div style={{ background: '#111', padding: '20px', borderRadius: '20px', marginBottom: '20px', border: '1px solid #222' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                    ...
                    <button onClick={() => setShowTableModal(true)} style={{ background: '#222', border: 'none', color: 'white', padding: '8px 15px', borderRadius: '8px', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer' }}>Edit</button>
                </div>
                <div style={{ paddingTop: '15px', borderTop: '1px dashed #333' }}>
                    ...
                    <input type="text" placeholder="Who is eating?" value={customerName} onChange={(e) => setCustomerName(e.target.value)}
