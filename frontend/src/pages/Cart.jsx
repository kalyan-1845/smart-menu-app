...
import { FaArrowLeft, FaTrash, FaCheckCircle, FaUtensils, FaPaperPlane } from "react-icons/fa";
...
const Cart = ({ cart, clearCart, removeFromCart, tableNum }) => {
    ...
    useEffect(() => {
        if (restaurantId) socketRef.current = io("https://smart-menu-app-production.up.railway.app", { transports: ['polling'] });
        return () => { if (socketRef.current) socketRef.current.disconnect(); };
    }, [restaurantId]);

    const handlePlaceOrder = async () => { 
        if (isSubmitting) return;
        if (!finalTableNum) { toast.error("Please scan the Table QR Code again."); return; }
        ...
        try {
            ...
            const payload = {
                ...
                items: cart.map(i => ({ name: i.name, quantity: i.quantity, price: i.price, dishId: i._id, image: i.image })),
    ...
    const handleAddMore = () => { clearCart(); navigate(`/menu/${restaurantId}?table=${finalTableNum}`); };
            ...
            {orderSuccess && (
                <div style={styles.modalOverlay}>
                    <div style={styles.modalCard}>
                        <div style={styles.popupHeader}><h3 style={styles.popupBrand}>KOVIXA <span style={{color:'#f59e0b'}}>x</span> {restaurantId?.toUpperCase()}</h3></div>
                        <div style={styles.successIconBox}><FaCheckCircle size={50} color="#22c55e" /></div>
                            ...
                            {cart.map((item) => (
                                <div key={item._id} style={styles.receiptItem}>
                                    <div style={{display:'flex', gap:8, alignItems:'center'}}>
                                        <div style={styles.qtySquare}>{item.quantity}</div><span style={styles.receiptName}>{item.name}</span>
                                    </div>
                                    <span style={styles.receiptPrice}>₹{item.price * item.quantity}</span>
                                </div>
                            ))}
                            <div style={styles.divider}></div>
                            <div style={styles.receiptTotal}><span>Total Amount</span><span style={{color:'#f59e0b'}}>₹{totalPrice}</span></div>
                        </div>
                        <button onClick={handleAddMore} style={styles.addMoreBtn}><FaUtensils /> ADD MORE ITEMS</button>
                    </div>
                </div>
            )}
            <div style={styles.header}>
                <button onClick={() => navigate(-1)} style={styles.backBtn}><FaArrowLeft /></button>
                <div><h1 style={styles.pageTitle}>Review Order</h1><p style={styles.pageSub}>Table {finalTableNum || "?"}</p></div>
            </div>
                    ...
                    cart.map(item => (
                        <div key={item._id} style={styles.cartItem}>
                            <div style={styles.itemInfo}>
                                ...
                                <div style={styles.itemMeta}><span style={styles.qtyBadge}>{item.quantity}x</span><span style={styles.itemPrice}>₹{item.price * item.quantity}</span></div>
                            </div>
                            <button onClick={() => removeFromCart(item._id)} style={styles.trashBtn}><FaTrash size={14} /></button>
                        </div>
                    ))
                )}
            </div>
            {cart.length > 0 && (
                <div style={styles.footerWrapper}>
                    <div style={styles.footerContent}>
                        <div style={styles.billRow}><span style={{color:'#94a3b8'}}>Total to Pay</span><span style={{fontWeight:'800', fontSize:18, color:'#f59e0b'}}>₹{totalPrice}</span></div>
                        <button onClick={handlePlaceOrder} disabled={isSubmitting} style={styles.placeOrderBtn}>{isSubmitting ? "SENDING..." : <>CONFIRM ORDER <FaPaperPlane /></>}</button>
                    </div>
                </div>
            )}
        </div>
...
const styles = {
    ...
    footerWrapper: { position: 'fixed', bottom: 0, left: 0, right: 0, background: 'rgba(2, 6, 23, 0.95)', backdropFilter: 'blur(10px)', borderTop: '1px solid #1e293b', zIndex: 100, paddingBottom: 'env(safe-area-inset-bottom)' },
