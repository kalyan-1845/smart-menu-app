...
const Menu = ({ cart, addToCart, setRestaurantId, setTableNum }) => {
    ...
    // 1. ✅ SAFE SYNC (Prevents Infinite Loop) + Persistence for Cart
    useEffect(() => {
        if (!currentRestId) return;
        
        // --- ADDED PERSISTENCE LOGIC START ---
        // We save these to localStorage so the Cart can always find them
        localStorage.setItem("last_restaurant_id", currentRestId);
        if (currentTable) localStorage.setItem("last_table_num", currentTable);
        // --- ADDED PERSISTENCE LOGIC END ---
