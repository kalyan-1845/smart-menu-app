...
const RestaurantAdmin = () => {
    ...
    const fetchData = async (token, mongoId) => {
        try {
            const res = await axios.get(`${API_BASE}/dishes?restaurantId=${mongoId}`, { headers: { Authorization: `Bearer ${token}` } });
            setDishes(res.data || []);
    ...
    const handleDeleteDish = async (dishId) => {
        ...
        const token = localStorage.getItem(`owner_token_${id}`);
        const mongoId = localStorage.getItem(`owner_id_${id}`);
        try { await axios.delete(`${API_BASE}/dishes/${dishId}`, { headers: { Authorization: `Bearer ${token}` } }); fetchData(token, mongoId); } catch (err) { alert("Failed"); }
    };
            ...
            <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div className="glass-card" style={{ textAlign: 'center', width: '320px' }}>
                    ...
                    <form onSubmit={handleLogin} style={{ marginTop: '20px' }}>
                        ...
                        <button type="submit" className="btn-primary" style={{width:'100%', padding:'14px', borderRadius:'12px', border:'none', color:'white', fontWeight:'700'}}>{authLoading ? "Unlocking..." : "Access Control"}</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div className="admin-container"><style>{styles}</style>
            <div className="max-w-wrapper">
                <header style={{marginBottom:'30px'}}>
                    ...
                    <button onClick={() => { localStorage.clear(); window.location.reload(); }} className="btn-glass" style={{float:'right', color:'#ef4444'}}><FaSignOutAlt /></button>
                </header>

                <SetupWizard dishesCount={dishes.length} pushEnabled={true} />
                ...
                <nav className="nav-tabs">
                    ...
                    <button onClick={() => { setActiveTab("inbox"); setHasNewOrder(false); }} className={`tab-btn ${activeTab === "inbox" ? 'active' : ''}`}>Inbox {inboxOrders.length > 0 && <span style={{background:'#FF9933', color:'black', padding:'2px 6px', borderRadius:'10px'}}>{inboxOrders.length}</span>} {hasNewOrder && <div className="pulse-dot"></div>}</button>
                </nav>

                {activeTab === "inbox" && (
                    <div className="glass-card">
                        <h2 style={{fontSize:'14px', fontWeight:900, marginBottom:'20px'}}><FaInbox color="#FF9933"/> NEW ORDERS</h2>
                        {inboxOrders.length === 0 ? <p style={{textAlign:'center'}}>Inbox Empty</p> : inboxOrders.map(order => (
                            <div key={order._id} className="inbox-card"><p style={{fontWeight:900, color:'#FF9933', margin:0}}>TABLE {order.tableNum}</p><p style={{fontSize:'12px'}}>{order.items.map(i => `${i.name} x${i.quantity}`).join(", ")}</p><p style={{fontWeight:900}}>Rs.{order.totalAmount}</p></div>
                        ))}
                    </div>
                )}
                    ...
                    <div className="glass-card">
                        ...
                        <form onSubmit={handleAddDish}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                ...
                                <select className="input-dark" value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })}><option>Starters</option><option>Main Course</option><option>Dessert</option><option>Drinks</option></select>
                            </div>
                            <button type="submit" className="btn-primary" style={{width:'100%', padding:'14px', borderRadius:'12px', border:'none', color:'white', fontWeight:'700'}}>Save Dish</button>
                        </form>
                        <div style={{ marginTop: '30px' }}>{dishes.map(dish => (<div key={dish._id} className="dish-item"><div><p style={{ fontWeight: 900, margin: 0 }}>{dish.name}</p><p style={{ margin: 0, fontSize: '11px', color: '#FF9933' }}>₹{dish.price}</p></div><button onClick={() => handleDeleteDish(dish._id)} style={{background:'none', border:'none', color:'#888'}}><FaTrash /></button></div>))}</div>
                    </div>
                )}
            </div>
        </div>
