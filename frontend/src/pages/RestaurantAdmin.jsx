...
const RestaurantAdmin = () => {
    ...
    const fetchInbox = async () => {
        ...
        try {
            const res = await axios.get(`${API_BASE}/orders/inbox`, { params: { restaurantId: mongoId } });
            setInboxOrders(res.data);
        } catch (err) { console.error("Inbox Rejection Check:", err.response?.data); }
    };
    ...
    useEffect(() => {
        if (isAuthenticated) {
            ...
            socket.on("new-order", () => { fetchInbox(); if (activeTab !== "inbox") setHasNewOrder(true); });
            socket.on("new-waiter-call", (data) => { new Audio("https://assets.mixkit.co/active_storage/sfx/2869/2869-preview.mp3").play().catch(()=>{}); setActiveAlerts(prev => [...prev, data]); });
    ...
    const handleDownloadAndClear = async () => {
        ...
        const tableData = inboxOrders.map((order, i) => [i + 1, order.tableNum, order.items.map(item => `${item.name} x${item.quantity}`).join(", "), `Rs.${order.totalAmount}`, new Date(order.createdAt).toLocaleTimeString()]);
        doc.autoTable({ startY: 30, head: [['#', 'Table', 'Items', 'Total', 'Time']], body: tableData });
        doc.save(`Receipts_${Date.now()}.pdf`);
        try { await axios.put(`${API_BASE}/orders/mark-downloaded`, { restaurantId: mongoId }); setInboxOrders([]); alert("Inbox Cleared!"); } catch (err) { alert("Error clearing database."); }
    };
    ...
    const generatePrintableQRs = () => {
        ...
        for (let i = qrRange.start; i <= qrRange.end; i++) {
            ...
            qrCodesHtml.push(`<div style="display:inline-block; margin:20px; padding:20px; border:2px dashed #ccc; text-align:center; font-family:sans-serif; border-radius:15px; width:220px;"><h2 style="margin:0 0 10px 0; color:#FF9933; font-size:18px;">${restaurantName.toUpperCase()}</h2><img src="${qrSrc}" width="180" height="180" /><p style="margin:10px 0 0 0; font-weight:bold; font-size:20px;">TABLE ${i}</p></div>`);
        }
        printWindow.document.write(`<html><body onload="window.print()">${qrCodesHtml.join('')}</body></html>`);
        printWindow.document.close();
    };

    const handleLogout = () => { setIsAuthenticated(false); localStorage.removeItem(`owner_token_${id}`); localStorage.removeItem(`owner_id_${id}`); };
            ...
            <div className="toast-container">
                {activeAlerts.map((alert, i) => (
                    <div key={i} className="toast-alert">
                        <div><p style={{ fontSize: '10px', fontWeight: 900 }}>🛎️ TABLE {alert.tableNumber}</p><p style={{ fontWeight: 900, fontSize: '16px' }}>{alert.type?.toUpperCase()}</p></div>
            ...
            <div className="max-w-wrapper">
                <header className="admin-header">
                    <div className="header-top">
                        <div><h1 className="shop-title">{restaurantName}</h1><div>{isPro ? <span className="badge-pro"><FaCrown /> PRO PLAN</span> : <span className="badge-pro" style={{ color: '#60a5fa', borderColor: '#60a5fa' }}>Trial Plan</span>}</div></div>
                    ...
                    <div className="menu-link-box">
                        ...
                        <div style={{display:'flex', gap:'5px'}}><button onClick={() => { navigator.clipboard.writeText(publicMenuUrl); alert("Copied!"); }} className="action-btn"><FaCopy /></button><a href={publicMenuUrl} target="_blank" rel="noreferrer" className="action-btn"><FaExternalLinkAlt /></a></div>
                    </div>
                ...
                <nav className="nav-tabs">
                    ...
                    <button onClick={() => { setActiveTab("inbox"); setHasNewOrder(false); }} className={`tab-btn ${activeTab === "inbox" ? 'active' : ''}`}>Inbox {inboxOrders.length > 0 && <span style={{background:'#FF9933', color:'black', padding:'2px 6px', borderRadius:'10px', marginLeft:'5px'}}>{inboxOrders.length}</span>} {hasNewOrder && <div className="pulse-dot"></div>}</button>
                    <button onClick={() => setActiveTab("settings")} className={`tab-btn ${activeTab === "settings" ? 'active' : ''}`}>Setup</button>
                </nav>

                {activeTab === "inbox" && (
                    <div className="glass-card">
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}><h2 style={{fontSize:'14px', fontWeight:900}}><FaInbox color="#FF9933"/> NEW ORDERS</h2><button onClick={handleDownloadAndClear} className="btn-glass" style={{background:'#FF9933', color:'black', border:'none'}}><FaDownload /> PDF & Clear</button></div>
                        {inboxOrders.length === 0 ? <p style={{textAlign:'center', padding:'20px'}}>Inbox Empty</p> : inboxOrders.map(order => (<div key={order._id} className="inbox-card"><p style={{fontWeight:900, color:'#FF9933', margin:0}}>TABLE {order.tableNum}</p><p style={{fontSize:'12px', margin:'5px 0'}}>{order.items.map(i => `${i.name} x${i.quantity}`).join(", ")}</p><p style={{fontSize:'11px', fontWeight:900}}>Total: Rs.{order.totalAmount}</p></div>))}
                    </div>
                )}
                ...
                {activeTab === "menu" && (
                    <div className="glass-card">
                        ...
                        <form onSubmit={handleAddDish}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}><input className="input-dark" type="number" value={formData.price} onChange={e => setFormData({ ...formData, price: e.target.value })} placeholder="Price ₹" required /><select className="input-dark" value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })}><option>Starters</option><option>Main Course</option><option>Dessert</option><option>Drinks</option></select></div>
                            <button type="submit" className="btn-primary">Save Dish</button>
                        </form>
                        <div style={{ marginTop: '30px' }}>{dishes.map(dish => (<div key={dish._id} className="dish-item"><div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}><div style={{ width: '50px', height: '50px', borderRadius: '10px', background: '#222', overflow: 'hidden' }}>{dish.image ? <img src={dish.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <FaUtensils color="#333" style={{margin:'15px'}}/>}</div><div><p style={{ fontWeight: 900, margin: 0, fontSize: '14px' }}>{dish.name}</p><p style={{ margin: 0, fontSize: '11px', color: '#FF9933', fontWeight: 900 }}>₹{dish.price} • {dish.category.toUpperCase()}</p></div></div><button onClick={() => handleDeleteDish(dish._id)} className="action-btn"><FaTrash /></button></div>))}</div>
                    </div>
                )}

                {activeTab === "settings" && (
                    <>
                        <div className="glass-card"><h2 style={{ fontSize: '14px', fontWeight: 900, marginBottom: '20px' }}><FaQrcode color="#FF9933" /> Bulk QR Generator</h2><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '20px' }}><div><label style={{ fontSize: '10px', color: '#666', fontWeight: 900, display: 'block', marginBottom: '5px' }}>START TABLE</label><input type="number" className="input-dark" value={qrRange.start} onChange={e => setQrRange({...qrRange, start: parseInt(e.target.value) || 1})} /></div><div><label style={{ fontSize: '10px', color: '#666', fontWeight: 900, display: 'block', marginBottom: '5px' }}>END TABLE</label><input type="number" className="input-dark" value={qrRange.end} onChange={e => setQrRange({...qrRange, end: parseInt(e.target.value) || 1})} /></div></div><button onClick={generatePrintableQRs} className="btn-primary">Generate & Print QRs</button></div>
                        <div className="glass-card"><h2 style={{ fontSize: '14px', fontWeight: 900, marginBottom: '10px' }}><FaBell color="#FF9933" /> Nightly Reports</h2><p style={{ fontSize: '11px', color: '#888', marginBottom: '15px' }}>Receive a sales summary every night at 11:59 PM.</p><input className="input-dark" placeholder="your@email.com" defaultValue={ownerEmail} onBlur={async (e) => { const email = e.target.value; const mongoId = localStorage.getItem(`owner_id_${id}`); if(email && mongoId) { try { await axios.put(`${API_BASE}/auth/update-email`, { restaurantId: mongoId, email }); alert("Email updated!"); } catch (err) { alert("Error updating email."); } } }} /></div>
                        <div className="glass-card"><h2 style={{ fontSize: '14px', fontWeight: 900, marginBottom: '10px' }}>Staff Alerts</h2><button onClick={() => Notification.requestPermission()} className="btn-primary">Enable Push Alerts</button></div>
                    </>
                )}
            </div>
        </div>
